...
许多数据库、缓存、路由器和存储系统使用近似集合隶属测试来决定给定的项是否在一个集合（通常是很大的）中，并伴随有一些小的假阳性概率。这个测试最广泛使用的数据结构是布隆过滤器[[3](#10. 参考资料)]，由于其内存效率而被广泛研究。布隆过滤器已经被用来：减少概率路由表中所需的空间[[25](#10. 参考资料)]；加速最长前缀匹配的IP地址[[9](#10. 参考资料)]；改进网络状态管理和监控[[24](#10. 参考资料), [4](#10. 参考资料)]；在数据包中编码多播转发信息[[15](#10. 参考资料)]；以及其他许多应用[[6](#10. 参考资料)]。
